...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
    ...
    const res = `https://api.lolhuman.xyz/api/${command}?apikey=Fikrii&text=${encodeURIComponent(text.substring(0, 151))}`
    const res2 = `https://xteam.xyz/attp?file&text=${encodeURIComponent(text.substring(0, 151))}`
    let ztick = fs.readFileSync(`./media/qr.png`)
    try {
        if (command == 'attp' || command == 'attp2') {
            let stiker = await sticker(false, res, global.packname)
            await conn.sendFile(m.chat, stiker, 'atet.webp', '', m)
        } else if (command == 'attp3') {
            let stiker = await sticker(false, res2, global.packname)
            await conn.sendFile(m.chat, stiker, 'atet.webp', '', m)
        } else {
            const stiker = await sticker(false, res, global.packname)
            await conn.sendFile(m.chat, stiker, 'atet.webp', '', m)
        }
    } catch (e) {
        console.log(e)
